  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", background: "#f5f5f5" }}>
      ...
      <div style={{ display: "flex", flex: 1, padding: 24, gap: 20, height: "calc(100vh - 57px)", boxSizing: "border-box" }}>
        ...
        {/* Left Panel */}
        <div style={{
          flex: "0 0 28%", display: "flex", flexDirection: "column", gap: 16,
        }}>
          ...
          <div style={{
            ...
            <h3 style={{ margin: "0 0 14px 0", fontSize: "1rem", color: "#1a237e" }}>
              Pending Requests {pendingRequests.length > 0 && (
                <span style={{
                  background: "#e65100", color: "white", borderRadius: 12,
                  fontSize: "0.75rem", padding: "2px 8px", marginLeft: 8,
                }}>{pendingRequests.length}</span>
              )}
            </h3>
            <div style={{ overflowY: "auto", flex: 1 }}>
              {pendingRequests.length === 0 && (
                <p style={{ color: "#aaa", fontSize: "0.9rem", margin: 0 }}>No pending requests.</p>
              )}
              {pendingRequests.map((r) => (
                <div
                  ...
                  style={{
                    ...
                    background: "#fff8f5", border: "1px solid #ffe0cc", cursor: "pointer",
                  }}
                >
          ...
          <div style={{
            ...
            <h3 style={{ margin: "0 0 14px 0", fontSize: "1rem", color: "#1a237e" }}>Students</h3>
            <div style={{ overflowY: "auto", flex: 1 }}>
              {students.length === 0 && (
                <p style={{ color: "#aaa", fontSize: "0.9rem", margin: 0 }}>No students found.</p>
              )}
              {students.map((s) => (
                <div
                  key={s.id}
                  ...
                  style={{
                    ...
                    background: selectedStudent?.id === s.id ? "#e8eaf6" : "#fafafa",
                    border: selectedStudent?.id === s.id ? "1px solid #9fa8da" : "1px solid #e8e8e8",
                  }}
                >
                  <div style={{ fontWeight: "600", fontSize: "0.9rem" }}>
        ...
        <div style={{
          ...
          {!selectedStudent ? (
            <div style={{ color: "#bbb", fontSize: "1rem", marginTop: 40, textAlign: "center" }}>
            ...
            <>
              ...
              <div style={{ marginBottom: 28 }}>
                <h2 style={{ margin: "0 0 6px 0", fontSize: "1.4rem", color: "#1a237e" }}>
              ...
              {/* Degree Progress */}
              <div style={{ marginBottom: 28 }}>
                <h3 style={{ margin: "0 0 12px 0", fontSize: "1rem", color: "#333" }}>Degree Progress</h3>
                {!audit ? (
                  <p style={{ color: "#aaa", fontSize: "0.9rem" }}>Loading...</p>
                ) : (
                  <>
                    <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 14 }}>
                      <div style={{ flex: 1, background: "#e0e0e0", borderRadius: 8, height: 20, overflow: "hidden" }}>
                        <div style={{
                          background: "#43a047", height: "100%", borderRadius: 8,
                          width: `${progressPct}%`, transition: "width 0.6s ease",
                        }} />
                      </div>
                      <span style={{ fontSize: "0.9rem", whiteSpace: "nowrap", color: "#555" }}>
              ...
              {/* Change Requests */}
              <div style={{ marginBottom: 28 }}>
                <h3 style={{ margin: "0 0 12px 0", fontSize: "1rem", color: "#333" }}>Change Requests</h3>
                {studentRequests.length === 0 ? (
                  <p style={{ color: "#aaa", fontSize: "0.9rem" }}>No change requests.</p>
                ) : (
                    ...
                    <div key={r.id} style={{
                      ...
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        ...
                        <span style={{
                          ...
                          background: requestStatusColor(r.status), color: "white",
                        }}>
                          {r.status}
                        </span>
                      </div>
                      <div style={{ fontSize: "0.85rem", color: "#666", marginTop: 6 }}>
                        <span style={{ color: "#999" }}>From:</span> {r.currentValue}
                        <span style={{ margin: "0 8px", color: "#999" }}>→</span>
                        {r.proposedValue}
                      </div>
                      {r.advisorNotes && (
                        <div style={{ fontSize: "0.82rem", color: "#777", marginTop: 6, fontStyle: "italic" }}>
                          Note: {r.advisorNotes}
                        </div>
                      )}
              ...
              {/* Advising Notes */}
              <div>
                <h3 style={{ margin: "0 0 12px 0", fontSize: "1rem", color: "#333" }}>Advising Notes</h3>
                {notes.length === 0 && (
                  <p style={{ color: "#aaa", fontSize: "0.9rem" }}>No notes yet.</p>
                )}
                {notes.map((n) => (
                  <div key={n.id} style={{
                    ...
                    <div style={{ fontSize: "0.9rem" }}>{n.note}</div>
                    <div style={{ fontSize: "0.78rem", color: "#bbb", marginTop: 6 }}>
                ...
                {noteError && (
                  <p style={{ color: "red", fontSize: "0.85rem" }}>{noteError}</p>
                )}
